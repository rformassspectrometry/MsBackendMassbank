...
- Refactor
  ...
  adding parameters for the individual metadata blocks simplifying
...
- Allow columns/fields “comment” and “data_processing_comment” to have
  multiple values.
...
- Import method generics from `ProtGenerics`. This requires
  `ProtGenerics` version 1.35.3.

### Changes in 1.11.1

- Remove additional empty line at the end of exported MassBank records
  ([issue](https://github.com/rformassspectrometry/MsBackendMassbank/issues/49)
...
- Add `backendBpparam` for `MsBackendMassbankSql`; parallel processing
  of `Spectra` with `MsBackendMassbankSql` will silently disable
  parallel processing.
...
- Fix an issue in which `selectSpectraVariables,MsBackendMassbankSql`
  ...
  [issue](https://github.com/rformassspectrometry/MsBackendMassbank/issues/29)
